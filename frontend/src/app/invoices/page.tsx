...
export default function InvoicesPage() {
  ...
  const handleWhatsApp = async (inv: any) => {
    ...
    try {
      ...
      // Trigger PDF download simultaneously
  ...
  const columns = [
    ...
    { key: 'total', header: 'Total', render: (r: any) => {
      const paid = (r.payments || []).filter((p: any) => !p.refunded).reduce((s: number, p: any) => s + Number(p.amount), 0);
    ...
    { key: 'actions', header: '', render: (r: any) => (
      <div className="flex items-center gap-1">
        ...
        <button
          onClick={async () => { toast.loading('Preparing PDF…',{id:`dl-${r.id}`}); try { await downloadFile(`/api/invoices/${r.id}/pdf/download`); toast.success('Downloading',{id:`dl-${r.id}`}); } catch(e){ toast.error('Download failed',{id:`dl-${r.id}`}); } }}
