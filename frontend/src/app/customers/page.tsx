...
export default function CustomersPage() {
  ...
  const openHistory = (c: any) => { setSelected(c); setModal('history'); };
  ...
  const columns = [
    { key:'name', header:'Name', render:(r:any) => <span className="font-semibold text-gray-900">{r.name}</span> },
    { key:'phone', header:'Phone', render:(r:any) => <span className="flex items-center gap-1.5"><Phone className="w-3 h-3 text-gray-400 flex-shrink-0"/><span className="text-sm">{r.phone}</span></span> },
    { key:'email', header:'Email', className:'hidden md:table-cell', render:(r:any) => r.email ? <span className="flex items-center gap-1.5"><Mail className="w-3 h-3 text-gray-400 flex-shrink-0"/><span className="text-sm truncate max-w-[140px]">{r.email}</span></span> : <span className="text-gray-300">—</span> },
    { key:'createdAt', header:'Added', className:'hidden lg:table-cell', render:(r:any) => <span className="text-sm text-gray-500">{formatDate(r.createdAt)}</span> },
