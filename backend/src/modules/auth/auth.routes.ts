...
userRouter.delete('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const user = await prisma.user.findUnique({ where: { id: req.params.id } });
    if (!user) throw new NotFoundError('User');
    // Unlink repair jobs where this user is the technician, and remove their
    // audit log entries — both have FK constraints with no cascade that would
    // cause Postgres to block the deletion otherwise.
    await prisma.repairJob.updateMany({ where: { technicianId: req.params.id }, data: { technicianId: null } });
